...
export default function FundCard({ fund, estimate, accountId }) {
  ...
  return (
    <div
      ...
      className="active:bg-gray-50 cursor-pointer transition-colors py-4"
    >
      {/* 基金名和持有金额 */}
      <div className="flex justify-between items-center mb-2">
        <div className="text-base text-gray-800 font-medium truncate mr-4">
          {estimate?.name || fund.name || fund.code}
        </div>
        <div className="text-sm text-gray-400 whitespace-nowrap">¥{fund.amount.toFixed(2)}</div>
      </div>
      ...
      <div className="flex">
        <div className="flex-1">
          {dailyProfit !== null ? (
            <>
              <div className={`text-base font-semibold ${dailyGrowth >= 0 ? 'text-profit' : 'text-loss'}`}>
